  ...
  useEffect(() => {
    const loadBaseUrl = async () => {
      try {
        ...
        // Silently handle errors in Cloudflare Worker environment
        console.warn('Failed to load baseUrl:', error)
      }
    }
    loadBaseUrl()
  }, [])
      ...
      try {
        ...
        try {
          if (typeof window !== 'undefined' && window.dispatchEvent) {
            window.dispatchEvent(new CustomEvent('mpc:baseUrl', { detail: baseUrl }))
          }
        } catch {}
      } catch (error) {
        // Silently handle errors in Cloudflare Worker environment
        console.warn('Failed to save baseUrl:', error)
      }
    }
    saveBaseUrl()
  }, [baseUrl])
  ...
  useEffect(() => {
    const loadHeaderVars = async () => {
      try {
        ...
        // Silently handle errors in Cloudflare Worker environment
        console.warn('Failed to load headerVars:', error)
      }
    }
    loadHeaderVars()
  }, [])
